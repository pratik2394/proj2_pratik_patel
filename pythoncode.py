...
counter = 1         #This is to be updated at every place we are using counter. IMPORTANT 
#or may be I could use len(open_list) + len(closed_list)
...
def create_childnode(the_node,open_list, closed_list):
    for action in action_sets:
        ...
        #potentially_new_node_coords = [the_node[1][0]+action[0], the_node[1][1]+action[1]]
...
def queue_processing_function(open_list, closed_list):
  ...
  #  if queue_for_procesing:
    while open_list:
        ...
        highest_priority_node = open_list.pop(temp_cost_list.index(min(temp_cost_list)))      
...
def generate_path(node_reached_goal, path_to_goal):
    masked_img[node_reached_goal[1][0], node_reached_goal[1][1]] = [0, 200, 0]
    cv2.imshow('Exploration Visualization', masked_img)
    cv2.waitKey(50) 
    if node_reached_goal[1] == goal_state:
        cv2.waitKey(0) & 0xFF == ord('q') 
        cv2.destroyAllWindows()
    if node_reached_goal[0] == 1:
         path_to_goal.append(1)
         return None
     #Using recursion:
    else:                       
        path_to_goal.append(node_reached_goal[2])             #Appending parent index to the path_to_goal
        tempP2G = [closed_list[tempPG][0] for tempPG in range(len(closed_list))].index(node_reached_goal[2])        
...
#[self_node_number, selfNodeCoords[y, x], parent node, parentNodeCoords[y, x], cost_to_come] 
